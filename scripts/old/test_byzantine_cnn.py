#!/usr/bin/env python3
"""Test Byzantine detection with CNN model (10×128 final layer)"""
...
print("="*80)
print("BYZANTINE DETECTION - CNN Model (10×128)")
print("="*80)

# Create CNN model
model = create_mnist_model(model_type='cnn', num_classes=10)
weights = model.fc2.weight.data.clone()  # Final FC layer: 10×128
...
# Test 2: Malicious CNN (perturbed weights)
print(f"\n{'='*80}")
print("TEST 2: Malicious CNN Model (perturbed weights)")
print(f"{'='*80}")
...
honest_fp, _ = prover.compute_model_fingerprint(weights, bias, r_vec, round_number, sample_size=100)
malicious_fp, _ = prover.compute_model_fingerprint(malicious_weights, bias, r_vec, round_number, sample_size=100)
...
print("SUMMARY")
print(f"{'='*80}")
print(f"{'✅' if honest_passed else '❌'} Honest CNN: {'PASS' if honest_passed else 'FAIL'}")
print(f"{'✅' if malicious_caught else '❌'} Malicious CNN: {'PASS' if malicious_caught else 'FAIL'}")
print(f"\nDetection Rate: {100 if malicious_caught else 0}%")

if honest_passed and malicious_caught:
    print("\n🎉 CNN MODEL PASSED - 100% Byzantine detection!")
    sys.exit(0)
else:
    print("\n⚠️  SOME TESTS FAILED")
    sys.exit(1)
